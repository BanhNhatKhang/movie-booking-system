...
<div class="container py-4 content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1><i class="bi bi-person-gear me-2"></i>Sửa thông tin nhân viên</h1>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="/sua-nhan-vien?id={{ $nhanVien['nd_id'] }}">
                        <input type="hidden" name="id" value="{{ $nhanVien['nd_id'] }}">
                        
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label">ID nhân viên</label>
                                <input type="text" class="form-control" value="{{ $nhanVien['nd_id'] }}" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Tên đăng nhập</label>
                                <input type="text" class="form-control" value="{{ $nhanVien['nd_tendangnhap'] ?? 'N/A' }}" readonly>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="nd_hoten" 
                                   value="{{ $nhanVien['nd_hoten'] }}" required>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="email" class="form-control" name="nd_email" 
                                       value="{{ $nhanVien['nd_email'] }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Số điện thoại</label>
                                <input type="text" class="form-control" name="nd_sdt" 
                                       value="{{ $nhanVien['nd_sdt'] ?? '' }}" placeholder="0123456789">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="nd_gioitinh">
                                    ...
                                    <option value="1" {{ ($nhanVien['nd_gioitinh'] ?? '') == '1' ? 'selected' : '' }}>Nam</option>
                                    <option value="0" {{ ($nhanVien['nd_gioitinh'] ?? '') == '0' ? 'selected' : '' }}>Nữ</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Ngày sinh</label>
                                <input type="date" class="form-control" name="nd_ngaysinh" 
                                       value="{{ $nhanVien['nd_ngaysinh'] ?? '' }}">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="nd_role" 
                                        {{ $nhanVien['nd_id'] == $_SESSION['user_id'] ? 'disabled' : '' }}>
                                    <option value="user" {{ ($nhanVien['nd_role'] ?? 'admin') === 'user' ? 'selected' : '' }}>
                                        User (Khách hàng)
                                    </option>
                                    <option value="admin" {{ ($nhanVien['nd_role'] ?? 'admin') === 'admin' ? 'selected' : '' }}>
                                        Admin (Quản trị viên)
                                    </option>
                                </select>
                                @if($nhanVien['nd_id'] == $_SESSION['user_id'])
                                    <small class="text-muted">Bạn không thể thay đổi role của chính mình</small>
                                    <input type="hidden" name="nd_role" value="{{ $nhanVien['nd_role'] }}">
                                @endif
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="nd_trangthai"
                                        {{ $nhanVien['nd_id'] == $_SESSION['user_id'] ? 'disabled' : '' }}>
                                    <option value="active" {{ ($nhanVien['nd_trangthai'] ?? 'active') === 'active' ? 'selected' : '' }}>
                                        Hoạt động
                                    </option>
                                    <option value="locked" {{ ($nhanVien['nd_trangthai'] ?? 'active') === 'locked' ? 'selected' : '' }}>
                                        Đã khóa
                                    </option>
                                </select>
                                @if($nhanVien['nd_id'] == $_SESSION['user_id'])
                                    <small class="text-muted">Bạn không thể khóa tài khoản của chính mình</small>
                                    <input type="hidden" name="nd_trangthai" value="{{ $nhanVien['nd_trangthai'] }}">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="nd_cccd" 
                                   value="{{ $nhanVien['nd_cccd'] ?? '' }}" placeholder="Số CCCD/CMND">
                        </div>
                            ...
                            @if($nhanVien['nd_id'] != $_SESSION['user_id'])
                                <div>
                                    <button type="button" class="btn btn-warning btn-sm" 
                                            data-user-id="{{ $nhanVien['nd_id'] }}" 
                                            data-user-name="{{ $nhanVien['nd_hoten'] }}"
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        @if(($nhanVien['nd_role'] ?? 'admin') === 'admin')
                            <span class="badge bg-danger">
                    ...
                    <div class="d-grid gap-2">
                        <a href="/chi-tiet-nhan-vien?id={{ $nhanVien['nd_id'] }}" class="btn btn-outline-info btn-sm">
                            <i class="bi bi-eye me-2"></i>Xem chi tiết
                        </a>
                        
                        @if($nhanVien['nd_id'] != $_SESSION['user_id'])
                            <a href="/khoa-nhan-vien?id={{ $nhanVien['nd_id'] }}" 
                               class="btn btn-outline-{{ ($nhanVien['nd_trangthai'] ?? 'active') === 'active' ? 'danger' : 'success' }} btn-sm"
...
@section('page-js')
<script src="/static/js/admin/SuaNhanVien.js"></script>
@endsection